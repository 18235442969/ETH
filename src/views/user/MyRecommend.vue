<template>
  <div class="myRecommend" id="myRecommend">
    <div class="myInfo">
...
import { getUserChilds } from '../../api/user.js';
...
export default {
  ...
  methods: {
    ...
    initScroll() {
      //初始化滚动加载
      const that = this
      document.getElementById('myRecommend').onscroll= function(){
        const bodyHeight = document.getElementById('myRecommend').scrollHeight;
        const clientHeight = document.getElementById('myRecommend').clientHeight;
        const scrollAllHeight = bodyHeight - clientHeight;
        const scrollTop = document.getElementById('myRecommend').scrollTop;
     ...
     * [获取我的推荐]
     */
    getUserChilds: async function() {
      try {
        let res = await getUserChilds({
          pi: this.pageIndex
        });
